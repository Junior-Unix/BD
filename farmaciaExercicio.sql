...
CREATE TABLE IF NOT EXISTS `farmacia`.`produtos` (
  ...
  INDEX `fk_fabricante_2_idx` (`id_fabricante` ASC) VISIBLE,
  CONSTRAINT `fk_produtos_1`
    FOREIGN KEY (`id_tipo_produto`)
    REFERENCES `farmacia`.`tipos_produto` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_fabricante_2`
...
CREATE TABLE IF NOT EXISTS `farmacia`.`compras` (
  ...
  PRIMARY KEY (`id`),
  INDEX `fk_id_cliente_1_idx` (`id_cliente` ASC) VISIBLE,
...
CREATE TABLE IF NOT EXISTS `farmacia`.`receitas_medica` (
  ...
  PRIMARY KEY (`id`),
  UNIQUE INDEX `id_produto_compra_UNIQUE` (`id_produto_compra` ASC) VISIBLE,
